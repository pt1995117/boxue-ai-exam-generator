...
- **命题城市情景量级（business_realism）**：若「命题城市」已指定且非「未指定」占位说明，需判断题干/选项中的**情景数据**（如住宅总价、面积、佣金/税费/贷款额度量级等）是否与该城市常见市场情况**大致相符**。不要求与真实成交一致，允许取整；仅**明显离谱**时写入 `business_realism.issues`。**教材规则规定的比例、成数、税率、系数、年限等数字为规则锁定项，不得仅因城市量级而判为不真实**。
- 若命题城市为「未指定」，不做针对具体城市的量级核对，保留通用业务常识判断。
