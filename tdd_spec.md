...
- **TP25.5 流式输出**: 应能实时接收事件
  ...
  - 测试数据: 执行完整工作流
- **TP25.6 映射审核状态机（双端口径）**: Admin-Web 与 Streamlit 映射审核状态需分别可用
  - 断言: Admin-Web 映射核对使用`pending/approved`；Streamlit 映射核对使用`auto_pending/confirmed/rejected/remapped`
  - 测试数据: 同一教材在两端页面加载映射列表
- **TP25.7 映射审核批量口径一致（按端）**: 各端批量提交在各自状态机内保持语义一致
  - 断言: Admin-Web 提交`confirm_status=approved/pending`、Streamlit 提交`confirmed/rejected/remapped`后，后端展示状态正确
  - 测试数据: 两端分别执行批量提交
