...
# 构建 Critic 的 prompt
blind_question = {k: v for k, v in test_question.items() if k not in ['正确答案', '解析', 'answer', 'explanation']}
...
try:
    ...
    critic_answer = result.get("critic_answer", "UNKNOWN")
    ...
    print(f"\n5. 综合判定:")
    if (reverse_solve and can_deduce and 
    ...
    if result.get("deduction_process"):
        print(f"\n6. 推导过程:")
        print(f"   {result['deduction_process']}")
