...
graph TD
    ...
    Router -->|计算题/含公式<br/>retry_count=0| Calculator[Calculator Node<br/>1. 生成计算代码 ✅<br/>2. 执行计算 ✅<br/>3. 生成初稿]
    Router -->|法律类/综合类/非计算<br/>retry_count=0| Specialist[Specialist Node<br/>1. 照猫画虎<br/>2. 生成初稿]
    
    Calculator --> Writer[Writer Node<br/>格式化标准化]
    Specialist --> Writer
    
    Writer --> Critic[Critic Node<br/>质量验证<br/>参数提取 ✅<br/>代码规则校验 ✅<br/>计算步骤验证 ✅]
    
    Critic -->|通过| End([✅ 输出题目])
    Critic -->|需修复<br/>retry_count<3| Decision{修复决策}
    Critic -->|retry_count≥3| SelfHeal[自愈：结束输出]
...
Router Node
    ...
    ├─ 计算金融/法律相关度
    ├─ 提取掌握程度
    └─ 决定: CalculatorAgent / LegalAgent / GeneralAgent
    ↓
Specialist/Calculator Node
    ├─ 照猫画虎 (获取 examples)
    ├─ [Calculator only] 计算步骤分析 ✅
    │   ├─ 理解计算器可能只是解决一个步骤
    │   ├─ 分析是否需要多步计算
    │   └─ 提取具体数值参数（不能是描述性文字）✅
    ├─ [Calculator only] 决定是否调用计算器
    └─ 生成初稿 (draft)
...
workflow.add_edge("calculator", "writer")
...
```
Router → Specialist/Calculator → Writer → Critic
   ↑                                      ↓
   └──────────── reroute ────────────────┘
                                         ↓
                                      Fixer
...
7. ✅ **计算步骤分析**: Calculator Node 和 Critic Node 都能理解多步计算
