...
export default function AIGenerateTaskDetailPage() {
  ...
  const traceSignature = (trace) => {
    ...
    const lastSteps = Array.isArray(last.steps) ? last.steps : [];
    ...
    return [
      ...
      lastSteps.length,
      String(lastStep?.node || ''),
      String(lastStep?.message || ''),
      String(lastStep?.detail || ''),
      String(last?.saved ? '1' : '0'),
      String(last?.critic_result?.passed ?? ''),
    ].join('|');
  };
  ...
  const buildOrderedTraceDisplay = (steps) => {
  ...
  const getLatestRunStatus = (steps) => {
    ...
    return { latestRunId, latestRunHasCriticTerminal, latestRunHasFixer };
  };
    ...
    <Space direction="vertical" style={{ width: '100%' }} size={12}>
      ...
      <Card loading={loading}>
        <Descriptions size="small" bordered column={2}>
          ...
          <Descriptions.Item label="耗时">{calcDuration(task?.created_at, task?.ended_at)}</Descriptions.Item>
        ...
        <Collapse
          ...
          items={processTrace.map((item) => {
            ...
            return {
              ...
              label: (
                <Space>
                  <span>{`第 ${item.index} 题 | 切片 ${item.slice_id} | 耗时 ${Math.max(0, Math.round((item.elapsed_ms || 0) / 1000))}s`}</span>
          ...
          children: (
                <Space direction="vertical" style={{ width: '100%' }} size={12}>
                  ...
                  <div>
                    ...
                    <Space direction="vertical" style={{ width: '100%' }} size={12}>
                      {buildOrderedTraceDisplay(item.steps || []).map((entry, idx) => {
                  ...
                  {item.final_json && (
                    <Card
                      ...
                      title={
                        <Space>
                          <span>本题最终内容（入库版本）</span>
                          <Tooltip title="若经过多轮 Fixer 修复，此处与入库题目均为最后一轮通过 Critic 的版本，与步骤流水里第一次「修复」定稿可能不同。">
